...
async function init() {
  ...
  const selectedPhotographer = photographers.find(
    (photographer) => photographer.id === parseInt(paramsId, 10)
  );
  selectedPhotographerMedia = media.filter(
    (item) => item.photographerId === parseInt(paramsId, 10)
  );
  ...
  document
    ...
    .appendChild(photographerDetailsDOM);
  ...
  function navigate(direction) {
    if (direction === 'next') {
      ...
      activeImage =
        (activeImage - 1 + lightboxArray.length) % lightboxArray.length;
    }
